...
function seedFor(mode) {
  const base = {
    ...
    symptomLogs: Array.from({ length: 12 }, (_, i) => ({ id: uid(), profileId: S, date: day(11 - i), symptoms: { hotFlashes: Math.max(0, 3 - Math.floor(i / 4)), sleepChanges: Math.max(0, 3 - Math.floor(i / 5)), brainFog: i % 3 === 0 ? 2 : 1, fatigue: Math.max(0, 2 - Math.floor(i / 6)) }, flow: "", notes: "" })),
